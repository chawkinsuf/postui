...
/// One dense, single-line list row: no half-block pads, no 2-line pitch —
/// just a flat fill on `row_y` with an optional zebra stripe underneath and
/// hover/selected states layered on top. Successor to [`PillRow`] for lists
/// that want a denser row pitch (Task 15 converts the remaining callers).
pub struct ListRow {
    pub highlight: RowHighlight,
